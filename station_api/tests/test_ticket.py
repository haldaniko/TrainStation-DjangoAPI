...
class TicketTests(APITestCase):
    def setUp(self):
        ...
        self.station_a = Station.objects.create(
            name="Station A", latitude=40.7128, longitude=-74.0060
        )
        self.station_b = Station.objects.create(
            name="Station B", latitude=34.0522, longitude=-118.2437
        )
        ...
        self.departure_time = timezone.make_aware(
            datetime.strptime("2024-07-27 08:00:00", "%Y-%m-%d %H:%M:%S")
        )
        ...
        self.order = Order.objects.create(
            created_at=self.departure_time, user=self.user
        )
    ...
    def test_create_ticket(self):
        ...
        response = self.client.post(
            reverse("station:ticket-list"), payload, format="json"
        )
    ...
    def test_ticket_unique_together(self):
        ...
        ticket_1 = Ticket.objects.create(
            cargo=1, seat=3, journey=self.journey, order=self.order
        )
